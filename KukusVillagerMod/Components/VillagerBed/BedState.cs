...
namespace KukusVillagerMod.Components.VillagerBed
{
    ...
    class BedState : MonoBehaviour, Hoverable, Interactable
    {
        ...
        public string GetHoverText()
        {
            ...
            {
                return
                    $"Belongs to {VillagerGeneral.GetName(GetVillagerZDOID())}({Util.GetZDO(GetVillagerZDOID()).m_uid.id})";
            }
        ...
        {
            if (!VillagerGeneral.SELECTED_VILLAGER_ID.IsNone())
            {
                VillagerGeneral.AssignBed(VillagerGeneral.SELECTED_VILLAGER_ID, znv.GetZDO().m_uid);
                MessageHud.instance.ShowMessage(MessageHud.MessageType.Center,
                    $"Assigned bed {znv.GetZDO().m_uid.id} for {VillagerGeneral.GetName(VillagerGeneral.SELECTED_VILLAGER_ID)}");
